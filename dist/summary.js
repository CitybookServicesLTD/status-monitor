...
const calculate_status_1 = require("./helpers/calculate-status");
...
const generateSummary = async () => {
    ...
    for await (const site of config.sites) {
        ...
        const statuss = await calculate_status_1.getstatusPercentForSite(slug);
        ...
        pageStatuses.push({
            ...
            status: statuss.all,
            statusDay: statuss.day,
            statusWeek: statuss.week,
            statusMonth: statuss.month,
            statusYear: statuss.year,
            ...
            dailyMinutesDown: statuss.dailyMinutesDown,
        });
...
<!-- This summary is generated by CitybookServicesLTD (https://github.com/CitybookServicesLTD/status) -->
<!-- Do not edit this manually, your changes will be overwritten -->
<!-- prettier-ignore -->
| ${i18n.url || "URL"} | ${i18n.status || "Status"} | ${i18n.history || "History"} | ${i18n.responseTime || "Response Time"} | ${i18n.status || "CitybookServicesLTD"} |
| --- | ------ | ------- | ------------- | ------ |
${pageStatuses
...
${config.summaryEndHtmlComment || "<!--end: status pages-->"}${endText}`;
    ...
    if (`${owner}/${repo}` !== "CitybookServicesLTD/status") {
        // Remove CitybookServicesLTD logo and add heaading
        readmeContent = readmeContent
            .split("\n")
            .map((line, index) => {
            if (index === 0 && line.includes("https://CitybookServicesLTD.js.org"))
            ...
            if (line.includes("[![Summary CI](https://github.com") &&
                ...
                return `${line}\n\nWith [CitybookServicesLTD](https://CitybookServicesLTD.js.org), you can get your own unlimited and free status monitor and status page, powered entirely by a GitHub repository. We use [Issues](https://github.com/${config.owner}/${config.repo}/issues) as incident reports, [Actions](https://github.com/${config.owner}/${config.repo}/actions) as status monitors, and [Pages](${website}) for the status page.`;
            return line;
        })
        ...
        if (readmeContent.includes("[MIT](./LICENSE) © [Koj](https://koj.co)") ||
            ...
            if (readmeContent.includes("<!--start: description-->"))
                readmeContent = `${descriptionStartText}This repository contains the open-source status monitor and status page for ${name}, powered by [CitybookServicesLTD](https://github.com/CitybookServicesLTD/status).${descriptionEndText}`;
            // Change copyright
            readmeContent = readmeContent.replace("[MIT](./LICENSE) © [Koj](https://koj.co)", `[MIT](./LICENSE) © ${name}`);
            // Add powered by CitybookServicesLTD
            if (!config.skipPoweredByReadme) {
                readmeContent = readmeContent.replace("## 📄 License\n\n- Code: [MIT](./LICENSE)", "## 📄 License\n\n- Powered by: [CitybookServicesLTD](https://github.com/CitybookServicesLTD/status)\n- Code: [MIT](./LICENSE)");
            }
        }
        // Change badges
        readmeContent = readmeContent.replace(new RegExp("CitybookServicesLTD/status/(workflows|actions)", "g"), `${config.owner}/${config.repo}/$1`);
        ...
        try {
            ...
            if (!repoInfo.data.description && !config.skipDescriptionUpdate)
                await octokit.repos.update({
                    owner,
                    repo,
                    description: `📈 status monitor and status page for ${name
                        .split("]")[0]
                        .replace("[", "")}, powered by @CitybookServicesLTD`,
                });
            console.log("Current topics are", repoInfo.data.topics);
            if (!(repoInfo.data.topics || []).includes("CitybookServicesLTD") &&
    ...
    git_1.commit((config.commitMessages || {}).readmeContent ||
        ":pencil: Update summary in README [skip ci] [CitybookServicesLTD]", (config.commitMessages || {}).commitAuthorName, (config.commitMessages || {}).commitAuthorEmail);
    ...
    git_1.commit((config.commitMessages || {}).summaryJson ||
        ":card_file_box: Update status summary [skip ci] [CitybookServicesLTD]", (config.commitMessages || {}).commitAuthorName, (config.commitMessages || {}).commitAuthorEmail);
    git_1.push();
    if (!config.skipDeleteIssues) {
        // Find all the opened issues that shouldn't have opened
        // Say, CitybookServicesLTD found a down monitor and it was back up within 5 min
        ...
        for await (const issue of issuesRecentlyClosed.data) {
            if (issue.closed_at &&
                ...
                new Date(issue.closed_at).getTime() -
                    ...
                    900000 &&
                // It has 1 comment (the default CitybookServicesLTD one)
                issue.comments === 1) {
                try {
