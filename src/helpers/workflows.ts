...
export const getstatusMonitorVersion = async () => {
...
jobs:
  release:
    ...
    steps:
      ...
      - name: Generate graphs
        uses: CitybookServicesLTD/status-monitor@${await getstatusMonitorVersion()}
        with:
          command: "graphs"
        env:
    ...
    steps:
      ...
      - uses: peaceiris/actions-gh-pages@v3.7.3
        ...
        with:
          ...
          user_email: "${
            commitMessages.commitAuthorEmail || "73812536+CitybookServicesLTD-bot@users.noreply.github.com"
          }"
`;
};
...
export const statusCiWorkflow = async () => {
  const config = await getConfig();
  const workflowSchedule = config.workflowSchedule || {};
...
on:
  schedule:
    - cron: "${workflowSchedule.status || status_CI_SCHEDULE}"
  repository_dispatch:
    types: [status]
  workflow_dispatch:
jobs:
